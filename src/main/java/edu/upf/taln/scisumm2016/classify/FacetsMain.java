...
public class FacetsMain {
    ...
    public static void FacetsMainRun(String args[]) {
        if (args.length == 5) {
            ...
            AnnotationSet rpReferencesGS = rp.getAnnotations("GS_REFERENCES");
                    ...
                    while (cpAnnotatorsIterator.hasNext()) {
                        ...
                        if (cpCitationSentences.size() > 0) {
                            ...
                            while (rpSentencesIterator.hasNext()) {
                                ...
                                AnnotationSet rpGSReferences = rpReferencesGS.get(rpStartS, rpEndS);
                                if (rpGSReferences.size() > 0) {
                                    Iterator rpGSReferencesIterator = rpGSReferences.iterator();
                                    while (rpGSReferencesIterator.hasNext()) {
                                        Annotation rpReferenceGS = (Annotation) rpGSReferencesIterator.next();
                                        if (rpReferenceGS.getFeatures().get("id")
            ...
            if (!generateTraining) {
                ...
                ProcessedRCDocuments = Utilities.spitOutOutputAnnotations(output, ProcessedRCDocuments, rfolderName, 1, String.valueOf(1.0), outputFolder);
            }
