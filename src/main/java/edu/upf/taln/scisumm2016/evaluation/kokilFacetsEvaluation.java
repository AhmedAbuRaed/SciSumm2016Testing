 ...
 */
public class kokilFacetsEvaluation {
    public static void main(String args[]) {
        ...
        int facetFN = falseNegatives(goldFacets, outputFacets); // this one doesn't care for matches, because we consider only false results
    ...
    private static int truePositivesFacets(HashMap<String, Set<String>> goldFacets, HashMap<String, Set<String>> outputFacets, HashMap<String, Set<String>> goldMatches, HashMap<String, Set<String>> outputMatches) {
        ...
        for (String citance : goldFacets.keySet()) {
            ...
            Set<String> intersectionFacetSet = intersect(goldFacetSet, outputFacetSet);

            // but we need to remove the items that were not a match
            Set<String> intersectionMatchesSet = intersect(goldMatchesSet, outputMatchesSet);
            intersectionFacetSet = intersect(intersectionFacetSet, intersectionMatchesSet);
    ...
    private static int falsePositivesFacets(HashMap<String, Set<String>> goldFacets, HashMap<String, Set<String>> outputFacets, HashMap<String, Set<String>> goldMatches, HashMap<String, Set<String>> outputMatches) {
        ...
        for (String citance : goldFacets.keySet()) {
            ...
            Set<String> differenceFacetsSet = subtract(outputFacetsSet, goldFacetsSet);

            // but we have to consider as false positive something that was in the intersection but was not a match
            Set<String> intersectionFacetsSet = intersect(goldFacetsSet, outputFacetsSet);
            Set<String> intersectionMatchesSet = subtract(outputMatchesSet, goldMatchesSet); // this are the matches
            intersectionFacetsSet.removeAll(intersectionMatchesSet); // this will leave the ones with the correct facet, but were not really matches
            
            Set<String> falsePositives = union(differenceFacetsSet, intersectionFacetsSet);

            value += falsePositives.size();
        }
